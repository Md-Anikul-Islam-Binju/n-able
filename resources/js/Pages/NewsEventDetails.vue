<template>
    <header class="position-relative" style="height: 50vh; overflow: hidden;">
        ...
        <div class="position-absolute top-0 start-0 w-100 h-100 d-flex flex-column justify-content-center align-items-start text-white bg-dark bg-opacity-50 px-4">
            <div class="container">
                <h1 class="fw-bold">{{ newsEvent.title }}</h1>
            </div>
        </div>
    </header>
            ...
            <div class="row justify-content-center">
                ...
                <div class="col-lg-8">
                    <div class="mb-3 text-muted">
                        <i class='bx bxs-calendar'></i> {{ newsEvent.event_date }}
                    </div>

                    <div class="mb-4 ">
                        <img
                            :src="baseUrl + '/images/event/' + newsEvent.image"
                            class="img-fluid mb-3 rounded-3"
                            alt="Corporate Business Presentation"
                        >
                        <p  v-html="getTruncatedText(newsEvent.details)"></p>
                    </div>
                </div>
                ...
                <div class="col-lg-4 bg-white shadow p-3 mb-5 bg-body-tertiary rounded-3" style="height: 40rem">
                    ...
                    <!-- News Item -->
                    <div  v-for="(newsEvent, index) in newsEvents" :key="index" class="mb-3 d-flex">
                        <div class="me-3">
                            <img :src="baseUrl + '/images/event/' + newsEvent.image" alt="Cyber Summit" class="img-thumbnail" style="width: 150px; height: 90px; object-fit: cover;">
                        </div>
                        <div>
                            <h6 class="mb-1"><Link :href="'/newsEventDetails/' + newsEvent.id" class="text-decoration-none text-dark">{{ newsEvent.title }}</Link></h6>
                            <small class="text-muted"><i class='bx bxs-calendar'></i> {{ newsEvent.event_date }}</small>
                        </div>
                    </div>
